...
def _is_process_alive(pid: int) -> bool:
    ...
    try:
        if os.name == "nt":
            # On Windows, os.kill(pid, 0) can raise WinError 87 for some processes.
            # We use a more robust check for Windows.
            import ctypes
        ...
        # On Windows, PermissionError can also mean the process exists but we can't 'kill' it.
        # WinError 87 often means the process is already gone or invalid.
        return False
...
def _find_server_pid_by_process() -> int | None:
    ...
    **and** is a Python interpreter (to avoid matching shell wrappers).
    ...
    """
    if os.name == "nt":
        # /proc does not exist on Windows, so we cannot scan it.
        return None
...
def _start_foreground(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print(f"DEBUG: Starting uvicorn on {args.host}:{args.port}...")
        uvicorn.run(
            app,
            host=args.host,
            port=args.port,
        ...
        import traceback
        traceback.print_exc()
        logger.exception("Critical error during server startup")
        sys.exit(1)
    finally:
        _remove_pid_file()
