...
export function applyDefaultSeeds(db: DbLike): void {
  ...
  if (agentCount === 0) {
    const insertAgent = db.prepare(
      `INSERT INTO agents (id, name, name_ko, name_ja, department_id, role, cli_provider, avatar_emoji, personality)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    );
    // Development (3)
    insertAgent.run(randomUUID(), "Aria", "아리아", "アリア", "dev", "team_leader", "claude", "👩‍💻", "きめ細かいシニア開発者");
    insertAgent.run(randomUUID(), "Bolt", "볼트", "ボルト", "dev", "senior", "codex", "⚡", "素早いコーディング職人");
    insertAgent.run(randomUUID(), "Nova", "노바", "ノバ", "dev", "junior", "copilot", "🌟", "創造力あふれるジュニア");
    // Design (2)
    insertAgent.run(randomUUID(), "Pixel", "픽셀", "ピクセル", "design", "team_leader", "claude", "🎨", "デザインリーダー");
    insertAgent.run(randomUUID(), "Luna", "루나", "ルナ", "design", "junior", "gemini", "🌙", "感性豊かなUIデザイナー");
    // Planning (2)
    insertAgent.run(randomUUID(), "Sage", "세이지", "セージ", "planning", "team_leader", "codex", "🧠", "戦略アナリスト");
    insertAgent.run(randomUUID(), "Clio", "클리오", "クリオ", "planning", "senior", "claude", "📝", "データ駆動の企画者");
    // Operations (2)
    insertAgent.run(randomUUID(), "Atlas", "아틀라스", "アトラス", "operations", "team_leader", "claude", "🗺️", "運用の達人");
    insertAgent.run(randomUUID(), "Turbo", "터보", "ターボ", "operations", "senior", "codex", "🚀", "自動化の専門家");
    // QA/QC (2)
    insertAgent.run(randomUUID(), "Hawk", "호크", "ホーク", "qa", "team_leader", "claude", "🦅", "鋭い品質の番人");
    insertAgent.run(randomUUID(), "Lint", "린트", "リント", "qa", "senior", "codex", "🔬", "緻密なテスト専門家");
    // DevSecOps (2)
    insertAgent.run(randomUUID(), "Vault", "볼트S", "ヴォルトS", "devsecops", "team_leader", "claude", "🛡️", "セキュリティアーキテクト");
    insertAgent.run(randomUUID(), "Pipe", "파이프", "パイプ", "devsecops", "senior", "codex", "🔧", "CI/CDパイプラインの専門家");
    // QA Junior (1)
    insertAgent.run(randomUUID(), "DORO", "도로롱", "ドロ", "qa", "junior", "gemini", "🩷", "丁寧な品質管理ジュニア");
  ...
  // Migrate: add sort_order column & set correct ordering for existing DBs
  {
    ...
    const newAgents: [string, string, string, string, string, string, string][] = [
      ...
      ["Clio", "클리오", "planning", "senior", "claude", "📝", "データ駆動の企画者"],
      ["Turbo", "터보", "operations", "senior", "codex", "🚀", "自動化の専門家"],
      ["Hawk", "호크", "qa", "team_leader", "claude", "🦅", "鋭い品質の番人"],
      ["Lint", "린트", "qa", "senior", "opencode", "🔬", "緻密なテスト専門家"],
