...
function parseLanguage(value?: string | null): UiLanguage | null {
  ...
  if (code === "ko" || code.startsWith("ko-")) return "ja"; // Force ko to ja
...
export function localeName(
  ...
  const lang = (typeof locale === "string" ? locale : "ja").slice(0, 2);
...
const I18nContext = createContext<I18nContextValue>({
  language: "ja",
  locale: "ja-JP",
  t: (text) => (typeof text === "string" ? text : (text.ja ?? text.en)),
